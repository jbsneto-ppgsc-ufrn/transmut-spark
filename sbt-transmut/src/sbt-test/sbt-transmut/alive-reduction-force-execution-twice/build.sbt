...
TaskKey[Unit]("checkDifferentReportsFromTwoExecutionsTransmut") := {
    ...
    // Checks if the number of mutants removed in the two executions is different 
    val listFilesFirstTransmutFolder = IO.listFiles(firstTransmutFolder)
    val listFilesSecondTransmutFolder = IO.listFiles(secondTransmutFolder)

    val firstReportsFolder = listFilesFirstTransmutFolder.filter(f => f.getName.contains("reports") && f.isDirectory()).head
    val firstReportsFolderFiles = IO.listFiles(firstReportsFolder)
    val firstHTMLReportsFolder = firstReportsFolderFiles.filter(f => f.getName.contains("html") && f.isDirectory()).head
    val firstJSONReportsFolder = firstReportsFolderFiles.filter(f => f.getName.contains("json") && f.isDirectory()).head

    val secondReportsFolder = listFilesSecondTransmutFolder.filter(f => f.getName.contains("reports") && f.isDirectory()).head
    val secondReportsFolderFiles = IO.listFiles(secondReportsFolder)
    val secondHTMLReportsFolder = secondReportsFolderFiles.filter(f => f.getName.contains("html") && f.isDirectory()).head
    val secondJSONReportsFolder = secondReportsFolderFiles.filter(f => f.getName.contains("json") && f.isDirectory()).head

    val firstHTMLReportsFolderFiles = IO.listFiles(firstHTMLReportsFolder)
    val firstHTMLRemovedMutantsReportsFolder = firstHTMLReportsFolderFiles.filter(f => f.getName.contains("RemovedMutants") && f.isDirectory()).head
    val firstHTMLRemovedMutantsReportsFolderFiles = IO.listFiles(firstHTMLRemovedMutantsReportsFolder)

    val secondHTMLReportsFolderFiles = IO.listFiles(secondHTMLReportsFolder)
    val secondHTMLRemovedMutantsReportsFolder = secondHTMLReportsFolderFiles.filter(f => f.getName.contains("RemovedMutants") && f.isDirectory()).head
    val secondHTMLRemovedMutantsReportsFolderFiles = IO.listFiles(secondHTMLRemovedMutantsReportsFolder)

    assert(firstHTMLRemovedMutantsReportsFolderFiles.size != secondHTMLRemovedMutantsReportsFolderFiles.size)

    val firstJSONReportsFolderFiles = IO.listFiles(firstJSONReportsFolder)
    val firstJSONRemovedMutantsReportsFolder = firstJSONReportsFolderFiles.filter(f => f.getName.contains("RemovedMutants") && f.isDirectory()).head
    val firstJSONRemovedMutantsReportsFolderFiles = IO.listFiles(firstJSONRemovedMutantsReportsFolder)

    val secondJSONReportsFolderFiles = IO.listFiles(secondJSONReportsFolder)
    val secondJSONRemovedMutantsReportsFolder = secondJSONReportsFolderFiles.filter(f => f.getName.contains("RemovedMutants") && f.isDirectory()).head
    val secondJSONRemovedMutantsReportsFolderFiles = IO.listFiles(secondJSONRemovedMutantsReportsFolder)

    assert(firstJSONRemovedMutantsReportsFolderFiles.size != secondJSONRemovedMutantsReportsFolderFiles.size)
}