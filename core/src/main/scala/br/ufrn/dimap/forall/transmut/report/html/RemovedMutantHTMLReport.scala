...
object RemovedMutantHTMLReport {
  
  def generateRemovedMutantHtmlReportFile(directory: File, fileName: String, metrics: RemovedMutantMetrics) {
    val content = generateRemovedMutantHtmlReport(metrics)
    IOFiles.generateFileWithContent(directory, fileName, content)
  }

  def generateRemovedMutantHtmlReport(metrics: RemovedMutantMetrics) = {
    s"""<!doctype html>
       |<html lang="en">
       |<head>
       |<!-- Required meta tags -->
       ...
       |  <h4><a href="#" class="text-dark">Removed Mutant ID: ${metrics.mutantId}</a></h4>
       |</div> 
       ...
       |  <h4>Reduction Rule: ${metrics.reductionRuleName}</h4>
       |<hr class="my-4">
       |</div>
       |</div>
       |<!-- Original Code -->
  ...
  def generateRemovedMutantModalHtml(metric: RemovedMutantMetrics, inIndex: Boolean = false) = {
    s"""<div class="modal fade" id="modalMutant${metric.mutantId}" tabindex="-1" role="dialog" aria-labelledby="modalMutantLabel${metric.mutantId}" aria-hidden="true">
       ...
       |<h5 class="modal-title" id="exampleModalLabel">Removed Mutant ID: <a href="${if(inIndex) "" else "../"}RemovedMutants/Removed-Mutant-${metric.mutantId}.html" class="text-dark">${metric.mutantId}</a></h5>
       ...
       |<h5>Reduction Rule: ${metric.reductionRuleName}</h5>
       |<hr class="my-4">
       |<h5>Original Code: </h5>
       |<hr class="my-4">
