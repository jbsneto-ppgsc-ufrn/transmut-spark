...
trait MutantRunner {
  ...
  def runMutationTestProcess(metaMutant: MetaMutantProgramSource, equivalentMutants: List[Long], testOnlyLivingMutants: Boolean = false, livingMutants: List[Long] = List(), forceExecutionEnabled: Boolean = false, forceExecution: List[Long] = List()): (MetaMutantProgramSource, List[TestResult[MutantProgramSource]]) = {
    val originalResult = runOriginalTest(metaMutant.original)
    val list = originalResult match {
      case TestSuccess(_) => metaMutant.mutants.map(mutant => runMutantTest(mutant, equivalentMutants, testOnlyLivingMutants, livingMutants, forceExecutionEnabled, forceExecution))
  ...
  def runMutantTest(mutant: MutantProgramSource, equivalentMutants: List[Long], testOnlyLivingMutants: Boolean = false, livingMutants: List[Long] = List(), forceExecutionEnabled: Boolean = false, forceExecution: List[Long]): TestResult[MutantProgramSource]
